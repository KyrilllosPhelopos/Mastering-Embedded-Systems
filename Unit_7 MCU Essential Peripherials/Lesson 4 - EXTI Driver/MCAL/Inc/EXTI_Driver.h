...
#ifndef EXTRI_DRIVER_H_
#define EXTRI_DRIVER_H_
/*=============================================================================
 * 							INCLUDES
 * ===========================================================================*/
...
typedef struct
{
	uint16_t 		ETI_INPUTLineNumber;//Choose The Line Number To Use in EXTI
										//value based on  @ref EXTI_define

	GPIO_TypeDef* 	GPIO_PORT;//at which port the pin will be connected to

	uint16_t        GPIO_PIN;//which pin in the port

	uint8_t         IVT_IRQNumber;//the value of the IRQ in the IVT
								// its values exists on stm32f103c5.h
...
typedef struct
{
	EXTI_GPIO_Mapping_t 	EXTI_PIN; //specify the external interrupt GPIO Mapping
								  // this parameter must be based on @ref EXTI_GPIO_Mapping_def

	uint8_t               	Triger_Case;//specifics RISING or FALLING or Both trigger
										//this parameter must be based on @ref EXTI_TRIGGER_def
	...
	uint8_t    				IRQ_Enable;// Enable or Disable the EXTI IRQ (That will enable the IRQ mask in EXTI and also on the NVIC Interrupt contoller)
									   // This parameter must be set based on @ref EXTI_IRQ_ENABLE_define
 ...
 * ==========================================================================================*/
#define EXTI_TRAGGER_RISSING_EDGE 						0
#define EXTI_TRAGGER_FALLING_EDGE 						1
#define EXTI_TRAGGER_RISSING_AND_FALLING_EDGE			2


/*=============================================================================================
 * 										@ref EXTI_IRQ_ENABLE_define
 * ==========================================================================================*/
